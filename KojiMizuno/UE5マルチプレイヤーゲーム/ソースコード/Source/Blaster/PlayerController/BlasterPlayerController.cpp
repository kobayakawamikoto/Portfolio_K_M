...
void ABlasterPlayerController::BeginPlay()
{
	...
	BlasterHUD = Cast<ABlasterHUD>(GetHUD()); // Can be to use CharacterOverlay
...
void ABlasterPlayerController::CheckPing(float DeltaTime)
{
	...
	if (HighPingRunningTime > CheckPingFrequency)
	{
		...
		if (PlayerState)
		{
			...
			if (BlasterHUD && BlasterHUD->CharacterOverlay && BlasterHUD->CharacterOverlay->PingAmountText)
			{
				FString PingText = FString::Printf(TEXT("%d"), FMath::FloorToInt(CurrentPing));
				BlasterHUD->CharacterOverlay->PingAmountText->SetText(FText::FromString(PingText));
			}
		}
...
void ABlasterPlayerController::ServerCheckMatchState_Implementation()
{
	...
	if (GameMode)
	{
		...
		LevelStartingTime = GameMode->LevelStartingTime; // TODO: This valiable should call after game mode beginplay?
...
{
	// ゲーム開始時GetHUDが機能しないためelse側で初期値の処理を書く必要がある
	BlasterHUD = BlasterHUD == nullptr ? Cast<ABlasterHUD>(GetHUD()) : BlasterHUD;
...
void ABlasterPlayerController::SetHUDCarriedAmmo(int32 Ammo)
{
	...
	if (bHUDValid)
	{
		...
		BlasterHUD->CharacterOverlay->CarriedAmmoAmount->SetText(FText::FromString(CarriedText));
	}
	else
	{
...
void ABlasterPlayerController::SetHUDTime()
{
	...
	if (HasAuthority())
	{
		if (BlasterGameMode == nullptr) // Fix Server and client time sync
		{
...
// キャラクター(Pawn)クラスに作成せずにControllerで作成するのはいつでもメインメニュー画面を開けるようにするため
