...
/*
* プレイヤーそのもののコリジョンではなく、プレイヤーの軌道から生成されたコリジョン(残像)に対してのヒット確認
* サーバーの巻き戻し処理を行うか判断している
*/
FServerSideRewindResult ULagCompensationComponent::ConfirmHit(const FFramePackage& Package, ABlasterCharacter* HitCharacter, const FVector_NetQuantize& TraceStart, const FVector_NetQuantize& HitLocation)
{
	...
	if (World)
	{
		...
		if (ConfirmHitResult.bBlockingHit) // we hit the head, return early
		{
			...
			{
				UBoxComponent* Box = Cast<UBoxComponent>(ConfirmHitResult.GetComponent());
				if (Box)
				{
...
/*
* 弾のコリジョンではなく、新たに生成したコリジョンでヒット確認を行う
*/
FServerSideRewindResult ULagCompensationComponent::ProjectileConfirmHit(const FFramePackage& Package, ABlasterCharacter* HitCharacter, const FVector_NetQuantize& TraceStart, const FVector_NetQuantize100& InitialVelocity, float HitTime)
{
	...
	if (PathResult.HitResult.bBlockingHit) // we hit the head, return early
	{
		...
		{
			UBoxComponent* Box = Cast<UBoxComponent>(PathResult.HitResult.GetComponent());
			if (Box)
			{
				//DrawDebugBox(GetWorld(), Box->GetComponentLocation(), Box->GetScaledBoxExtent(), FQuat(Box->GetComponentRotation()), FColor::Red, false, 8.f);
			}
...
void ULagCompensationComponent::SaveFramePackage(FFramePackage& Package)
{
	...
	if (Character)
	{
		...
		for (auto& BoxPair : Character->HitCollisionBoxes)
		{
			...
			Package.HitBoxInfo.Add(BoxPair.Key, BoxInformation); // Add variable
		}
	}
}