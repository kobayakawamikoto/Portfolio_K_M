	...
	bDead(false)
{
	...
	//FPSのようなキャラ回転を得る場合はshootercharacterbpからusecontrolleryawをonにする
	// Configure character movement
	GetCharacterMovement()->bOrientRotationToMovement = false; // Character moves in the direction of input...
	GetCharacterMovement()->RotationRate = FRotator(0.f, 540.f, 0.f); // ... at this rotation rate
	GetCharacterMovement()->JumpZVelocity = 600.0f; // Jump Power
	GetCharacterMovement()->AirControl = 0.2f; // 空中制御
...
// For Mouse sensitivity and rate
...
void AShooterCharacter::FireWeapon()
{
	...
	if (WeaponHasAmmo())
	{
		...
		StartFireTimer();
		// recoil
		AddControllerPitchInput(-30.f * GetWorld()->GetDeltaSeconds()); //deg/sec * sec/frame
...
bool AShooterCharacter::GetBeamEndLocation(const FVector& MuzzleSocketLocation, FHitResult& OutHitResult)
{
	...
	const FVector End = OutBeamLocation;
	if (bCrosshairHit)
	{
		...
		// 弾の拡散(エイム依存)
		float SpreadAmount = 64 * CrosshairSpreadMultiplier;
		OutBeamLocation += {
			FMath::FRandRange(-SpreadAmount, SpreadAmount), FMath::FRandRange(-SpreadAmount,
				SpreadAmount), 0
		};
	}
	...
	// Perform a second trace, this time from the gun barrel　銃身先端からendpointまでのベクトルでトレース　描画はしてない
	const FVector WeaponTraceStart{ MuzzleSocketLocation };
	const FVector StartToEnd{ OutBeamLocation - MuzzleSocketLocation };
	FVector WeaponTraceEnd{ OutBeamLocation + StartToEnd * 1.25f};
	//const FVector RotationAxis{ MuzzuleSocketRotation.GetAxisX() };
	//const FVector End{ WeaponTraceStart + RotationAxis * 50'000.f };
	...
	if (!OutHitResult.bBlockingHit) // 物に触れるとtrue // was there a trace hit?
	{

		OutHitResult.Location = OutBeamLocation; // 着弾点更新
		return false;
	}
	else
	{
		OutBeamLocation = End; // 着弾点再更新
		return true;
	}
	return true;
}
...
void AShooterCharacter::CalculateCrosshairSpread(float DeltaTime)
{
	...
	FVector Velocity{ GetVelocity() }; // Get Player Movement Velocity 
	Velocity.Z = 0.f;
	...
	/*
	float DebugVelocitySize = CrosshairVelocityFactor;
	FString StringVelocitySize = FString::Printf(TEXT("%f"), DebugVelocitySize);
	UKismetSystemLibrary::PrintString(this, StringVelocitySize, true, true, FColor(255, 255, 255), 10.f);*/
...
void AShooterCharacter::SendBullet()
{
	...
	if (BarrelSocket)
	{
		const FTransform SocketTransform = BarrelSocket->GetSocketTransform(EquippedWeapon->GetItemMesh()); //socket potision setting

		if (EquippedWeapon->GetMuzzleFlash())
		{
			UGameplayStatics::SpawnEmitterAtLocation(GetWorld(), EquippedWeapon->GetMuzzleFlash(), SocketTransform); // fire particle
		}
		...
		if (FireHit.bBlockingHit) //if hit something, true
		{
...
void AShooterCharacter::Tick(float DeltaTime)
{
	...
	// Change look sensitivity based on aiming(Gamepad only)
	SetLookRate();
	// Calculate crosshair spread multiplier
	CalculateCrosshairSpread(DeltaTime);
	// Check OverlappedItemCount, then trace for items
	TraceForItems();
	//if (GEngine) GEngine->AddOnScreenDebugMessage(1, -1, FColor::Red, FString::Printf(TEXT("Crouch: %b"), bCrouching));
...
void AShooterCharacter::GetPickItem(AItem* Item)
{
	Item->PlayEquipSound();
	...
	if (Weapon)
	{
		...
		else // Invetory is full! Swap with EquippedWeapon
		{
			SwapWeapon(Weapon);
		}
	}
