...
void AItem::BeginPlay()
{
	...
	//Mluti AreaSphere->OnComponentBeginOverlap.AddDynamic(this, &AItem::OnSphereOverlap);
	//Mluti AreaSphere->OnComponentEndOverlap.AddDynamic(this, &AItem::OnSphereEndOverlap);
	...
	if (HasAuthority()) // GetLocalRole() == ENetRole::ROLE_Authority, サーバー上で一度だけデリゲートを送信すればよい
	{
...
void AItem::ItemInterp(float DeltaTime)
{
	...
	if (Character && ItemZCurve)
	{
		...
		// Elapsed time since we started ItemInterpTimer
		const float ElapsedTime = GetWorldTimerManager().GetTimerElapsed(ItemInterpTimer);
		const float InterpolationPosition = ElapsedTime / ZCurveTime; // Clamp
		// Get curve value corresponding to ElapsedTime
		const float CurveValue = ItemZCurve->GetFloatValue(InterpolationPosition);
		// Get the item's initial location when the curve started
		FVector ItemLocation = ItemInterpStartLocation;
		// Get location in front of the camera
		...
		const FVector CurrentLocation{ GetActorLocation() };
		// Interpolated X value
		const float InterpXValue = FMath::FInterpTo(CurrentLocation.X, CameraInterpLocation.X, DeltaTime, 300.0f);
		// Interpolated Y value
		const float InterpYValue = FMath::FInterpTo(CurrentLocation.Y, CameraInterpLocation.Y, DeltaTime, 300.0f);
		...
		if (ItemScaleCurve)
		{
			const float ScaleCurveValue = ItemScaleCurve->GetFloatValue(InterpolationPosition);
...
void AItem::Tick(float DeltaTime)
{
	...
	// Get curve values from PulseCurve and dynamic material parameters
	UpdatePulse();
}
