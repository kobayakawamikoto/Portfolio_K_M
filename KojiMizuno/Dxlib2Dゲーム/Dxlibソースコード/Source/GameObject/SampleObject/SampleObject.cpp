...
void SampleObject::Update(float delta_seconds)
{
	...
	yadd.y += gravity.y + 10 * delta_seconds; // deltaSecondsで可変フレーム対応
...
void SampleObject::Draw(const Vector2D& screen_offset)
{
	...
	//DrawFormatString(0, 100, 100, "今日の天気は %f です", GetPosition().x);
	//DrawFormatString(0, 100, 100, "重力加速値: %f ", yadd.y);
...
void SampleObject::HitFromAbove(int j) // マップチップの上からキャラが当たった
{
	Vector2D vec;
	vec.x = prev_x;
	prev_y = static_cast<float>(j * 40 - 38); // y座標を固定する
	vec.y = prev_y;
	if (GetPosition().y != prev_y)
	{
		SetPosition(vec);
...
void SampleObject::Bleed()
{
	...
	if (isDead)
	{
		...
		for (int i = 0; i < blood_num; i++)
		{
			...
			// 血のアニメーションの為に新たなベロシティを作成して適用しています(addyadd)。
...
void SampleObject::HitFromAbove_blood(int j, int k) // マップチップの上から血が当たった
{
	Vector2D vec;
...
void SampleObject::HitFromBelow_blood(int j, int k) // マップチップの下から血が当たった
{
	Vector2D vec;
	vec.x = prev_blood_x[k];
...
void SampleObject::HitFromSide_blood(int k) // マップチップの横から血が当たった
{
	Vector2D vec;
	vec.y = bloodVec[k].y;
	...
	_x *= 10;
	vec.x = static_cast<float>(_x);
	if (bloodVec[k].x != _x) // (GetPosition().x == _x)の場合下から上にすり抜ける床になる
	{
		bloodVec[k].x = vec.x;
		bloodVec[k].y = vec.y;
	}
